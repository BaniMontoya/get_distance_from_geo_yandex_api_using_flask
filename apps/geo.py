...
KEY = "0000-0000-0000-0000-0000" # update with your api key 

router_geo = flask.Blueprint('router_geo', __name__)
...
def calculateDistanceFromMoscou(ySource, xSource):
    ...
    if polygon.contains(point):
        ...
        current_app.logger.info(" Google Maps link, " +
...
@router_geo.route('/getDistanceFromMoscou/<destinyAddress>/', methods=['GET'])
def getDistanceFromMoscou(destinyAddress):
    ...
    try:
        ...
        if distanceInDegrees == 0 and distanceInMeters == 0:
            res = {"Message": "Inside Moscow Ring Road"}
            current_app.logger.info(res)
            return flask.make_response(flask.jsonify(res), 200)

        else:
            res = {"distanceInMeters": distanceInMeters,
                   "distanceInDegrees": distanceInDegrees}
            current_app.logger.info(res)
            return flask.make_response(flask.jsonify(res), 200)
    except:
        res = {"Message": "Invalid address, you can fix and try again."}
        current_app.logger.info(res)
        return flask.make_response(flask.jsonify(res), 400)
